(function() {
  var CSON, EOL, Q, async, fs, getLangResource, getProperty, gutil, handleUndefined, langRegExp, options, path, replaceProperties, supportedType, through,
  ...
  getLangResource = (function() {
    var define, getCSONResource, getJSONResource, getJsResource, getResource, getResourceFile, langResource, require;
    define = function() {
      var al;
      al = arguments.length;
      if (al >= 3) {
    ...
    getResourceFile = function(filePath) {
      ...
      try {
        ...
        } else if (path.extname(filePath) === '.cson') {
    ...
    getCSONResource = function(filePath) {
      return define(CSON.parse(fs.readFileSync(filePath).toString()));
    };
        ...
        if (fs.statSync(langDir).isDirectory()) {
          ...
          return async.each(fileList, function(filePath, cb) {
            ...
            if (ref = path.extname(filePath), indexOf.call(supportedType, ref) >= 0) {
              ...
              res[path.basename(filePath).replace(/\.(j|c)s(on)?$/, '')] = getResourceFile(filePath);
            }
            return cb();
          }, function(err) {
            if (err) {
